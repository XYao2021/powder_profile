...
with a computing node (by default a Dell d430).
...
def b210_nuc_pair(idx, b210_node):  #remove idx because useless for this function
    """Add a computing node for all b210s or computing nodes for each b210 LAN if the b210s are also lack of CPU cycles"""
...
portal.context.defineParameter("x310_pair_nodetype",
                               ...
                               node_type[0],  # default value for original file is d430 (node_type[1])
                               node_type)

portal.context.defineParameter("add_x310_pair_nodetype",
                               "Additional computing node type paired with the X310 Radios",
                               portal.ParameterType.STRING,
                               node_type[1],
                               node_type)  #Xin Yao
...
portal.context.defineStructParameter("x310_radios", "OTA Lab X310 Radios", [],
                                     ...
                                     members=[
                                        portal.Parameter(
                                             ...
                                             lab_x310_names[0],
                                             lab_x310_names)
                                     ])  # No attributes from geni.portal.context class, but has same parameters as geni.portal.StructParameters class
...
# Now verify.
portal.context.verifyParameters()  # Calls to Context.parameterError/Context.parameterWarning(Context.makeParameterWarnings), this will spit out some nice JSON-formatted exception info on stderr
